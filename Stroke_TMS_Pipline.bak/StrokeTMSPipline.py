...
class rsfMRIProcessor:
    ...
    def format_transform(self, t1_pattern=r't1', t2_pattern=r't2', fm_pattern=r'fieldmap', me_pattern=r'me', echo_pattern=r'Echo\d', echo1_pattern=r'Echo1', echo2_pattern=r'Echo2', b0map_pattern=r'B0Map', dti_pattern=r'dti', ap_pattern=r'AP', pa_pattern=r'PA'):
        ft = FormatTransformer(self.raw_dir, self.data_dir, subj_id=self.subj_id)
        ft.multi_echo_format(t1_pattern=t1_pattern, t2_pattern=t2_pattern, fm_pattern=fm_pattern, me_pattern=me_pattern, echo_pattern=echo_pattern, echo1_pattern=echo1_pattern, echo2_pattern=echo2_pattern, b0map_pattern=b0map_pattern, dti_pattern=dti_pattern, ap_pattern=ap_pattern, pa_pattern=pa_pattern)
    ...
    def single_echo_pipline(self):
        # 1. 建临时文件夹
        # 2. fmriprep
    #     os.system('fmriprep ./fmriprep_tmp ./fmriprep_tmp/derivatives participant \
	...
	#  --no-reconall  \
	#  --output-spaces T1w \
	#  -w /home/poolab/HeJieShi/tmp')
        # 3. denoise
        tsv = 'sub-001_ses-001_task-sleep_desc-confounds_timeseries.tsv'
        confond = 'sub-001_ses-001_task-sleep_desc-confounds_timeseries.json'
        bold = 'sub-001_ses-001_task-sleep_space-T1w_desc-preproc_bold.nii'
        mask = 'sub-001_ses-001_task-sleep_space-T1w_desc-brain_mask.nii'
        clean_data = f'{self.sub_dir}/func/rest/session_1/run_1/clean_data.nii'
        ...
        # 4. 移动denoised func bold
        # 5. 删除临时文件夹
        # 6. 重命名
        os.system(f'mcflirt -dof 6 -stages 3 -plots -in {clean_data} -out {self.sub_dir}/func_se/rest/session_1/run_1/MCF')
        ...
        os.system('fslinfo clean_data.nii | grep pixdim4 >> tmp.txt')
        os.system('sed \'s/pixdim4\\s*//\' tmp.txt | grep [0-9]*\\.[0-9]* > TR.txt')
        os.system('rm tmp.txt')
        os.system(f'bash bold_resample.sh {self.sub_dir}/func/rest/session_1/run_1 {self.sub_dir}/func/xfms/rest/T1w_acpc_brain_func_mask.nii.gz {single_echo_data}') 
        os.system(f'bash {self.script_dir}/se_func_denoise_mgtr.sh {self.subj_id} {self.data_dir} {self.script_dir}')
        os.system(f'bash {self.script_dir}/vol2surf.sh {self.subj_id} {self.data_dir} {self.script_dir} {self.script_dir}/config/fmriprep_list.txt 1')
...
class DTIProcessor:
    ...
    def run(self):
        ...
        os.system(f'bash {self.dti_dir}/01_MRtrix_Preproc_AP_Direction_final.sh')    
...
class QualityControl:
    ...
    def add_spec(self):
        wb_list = [f'{self.sub_dir}/anat/T1w/T1w_acpc_dc_restore.nii.gz', 
                   ...
                   f'{self.sub_dir}/pfm/M1_TargetNetwork.dtseries.nii',
