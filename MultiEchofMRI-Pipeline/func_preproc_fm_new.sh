...
for s in $sessions ; do
	...
	# sweep the runs;
	for r in $runs ; do
		echo "$Subdir/func/unprocessed/field_maps/phasediff_S"$s"_R"$r".nii.gz"
		# check to see if this file exists or not;
		if [ -f "$Subdir/func/unprocessed/field_maps/phasediff_S"$s"_R"$r".nii.gz" ]; then
...
func () {
	...
	cp "$2"/func/unprocessed/field_maps/magnitude2_"$5".nii.gz "$4"/FM_mag_"$5".nii.gz
	cp "$2"/func/unprocessed/field_maps/phasediff_"$5".nii.gz "$4"/phasediff_"$5".nii.gz
	...
	if [[ $nVols > 1 ]] ; then 
		mcflirt -in "$4"/FM_mag_"$5".nii.gz -out "$4"/FM_mag_"$5".nii.gz
		fslmaths "$4"/FM_mag_"$5".nii.gz -Tmean "$4"/FM_mag_"$5".nii.gz
		mcflirt -in "$4"/phasediff_"$5".nii.gz -out "$4"/phasediff_"$5".nii.gz
		fslmaths "$4"/phasediff_"$5".nii.gz -Tmean "$4"/phasediff_"$5".nii.gz
	fi

	bet "$4"/FM_mag_"$5".nii.gz "$4"/magbrain1_"$5".nii.gz > /dev/null 2>&1
	fslmaths "$4"/magbrain1_"$5".nii.gz -ero "$4"/FM_mag_brain_"$5".nii.gz > /dev/null 2>&1
	rm "$4"/magbrain1_"$5".nii.gz
	fsl_prepare_fieldmap "$6" "$4"/phasediff_"$5".nii.gz "$4"/FM_mag_brain_"$5".nii.gz "$4"/FM_rads_"$5".nii.gz "$7"
...
func $MEDIR $Subdir $Subject $WDIR $AllFMs $MACHINE $deltaTE
