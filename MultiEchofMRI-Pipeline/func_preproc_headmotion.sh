...
func () {
	...
	time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------define some acq. parameters $3/func/rest/$5/TE.txt"
	...
	# merge the images;
	time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------$3/func/rest/$5/Rest_AVG.nii.gz $3/func/rest/$5/vols/AVG_*.nii.gz"
	...
	time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------use the first echo (w/ least amount of signal dropout) to estimate bias field;"
	# use the first echo (w/ least amount of signal dropout) to estimate bias field;
...
func $MEDIR $AtlasTemplate $Subdir $DOF $AllScans
