...
time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------sweep through sessions "
	...
	for r in $runs ; do 
		...
		for i in $te ; do
			...
			if [ ! -f "$Subdir"/func/unprocessed/rest/session_"$s"/run_"$r"/SBref_S"$s"_R"$r"_E"$n_te".nii.gz ]; then
				fslroi "$Subdir"/func/unprocessed/rest/session_"$s"/run_"$r"/Rest_S"$s"_R"$r"_E"$n_te".nii.gz "$Subdir"/func/unprocessed/rest/session_"$s"/run_"$r"/SBref_S"$s"_R"$r"_E"$n_te".nii.gz 0 1 
				echo 0 > "$Subdir"/func/rest/session_"$s"/run_"$r"/rmVols.txt
			fi
...
time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------sweep through sessions done"
...
time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------use BBRegister (BBR) to fine-tune the existing co-registration & output FSL style transformation matrix"
...
# count the number of sessionsdone
...
time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------parallel proc"
...
func $MEDIR $Subdir $Subject $DOF $AtlasTemplate $Sessions

time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------parallel done"
...
time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------matlab proc"
matlab -nodesktop -nosplash -r "temp; exit" > /dev/null 2>&1
rm -rf "$Subdir"/workspace/ > /dev/null 2>&1
