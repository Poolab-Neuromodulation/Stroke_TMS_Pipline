...
MEDIR="//home/cputest/Stroke/Code/MultiEchofMRI-Pipeline"
...
MaxIterations=500 
MaxRestarts=5
MACHINE=SIEMENS # GEHC_FIELDMAPHZ
# MACHINE=GEHC_FIELDMAPHZ
deltaTE=2.4

# set variable value that sets up environment
EnvironmentScript="/home/cputest/Stroke/Code/HCPpipelines/Examples/Scripts/SetUpHCPPipeline.sh" # Pipeline environment script
source ${EnvironmentScript}	# Set up pipeline environment variables and software

echo -e "\nMulti-Echo Preprocessing & Denoising Pipeline" 

time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------Processing the Field Maps"
...
#"$MEDIR"/func_preproc_fm.sh "$MEDIR" "$Subject" "$StudyFolder" "$NTHREADS" "$StartSession"
"$MEDIR"/func_preproc_fm_new.sh "$MEDIR" "$Subject" "$StudyFolder" "$NTHREADS" "$StartSession" "$MACHINE" "$deltaTE"

time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------Coregistering SBrefs to the Anatomical Image"
...
"$MEDIR"/func_preproc_coreg.sh "$MEDIR" "$Subject" "$StudyFolder" "$AtlasTemplate" "$DOF" "$NTHREADS" "$StartSession" 

time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------Correcting for Slice Time Differences, Head Motion, & Spatial Distortion"
...
"$MEDIR"/func_preproc_headmotion.sh "$MEDIR" "$Subject" "$StudyFolder" "$AtlasTemplate" "$DOF" "$NTHREADS" "$StartSession"

time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------Performing Signal-Decay Based Denoising"
echo -e "Performing Signal-Decay Based Denoising"

# perform signal-decay denoising; 
# "$MEDIR"/func_denoise_meica.sh "$Subject" "$StudyFolder" "$NTHREADS" "$MEPCA" "$MaxIterations" "$MaxRestarts" "$StartSession"
"$MEDIR"/func_denoise_meica_new.sh "$Subject" "$StudyFolder" "$NTHREADS" "$MEPCA" "$MaxIterations" "$MaxRestarts" "$StartSession"

time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------------Removing Spatially Diffuse Noise via MGTR"
echo -e "Removing Spatially Diffuse Noise via MGTR"

# remove spatially diffuse noise; 
"$MEDIR"/func_denoise_mgtr.sh "$Subject" "$StudyFolder" "$MEDIR" "$StartSession"
...
"$MEDIR"/func_vol2surf.sh "$Subject" "$StudyFolder" "$MEDIR" "$CiftiList" "$StartSession"
