...
func () {
	...
	time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------run the tedana workflow;"
	tedana -d "$1"/func/rest/"$6"/Rest_E*_acpc.nii.gz -e $(cat "$1"/func/rest/"$6"/TE.txt) --out-dir "$1"/func/rest/"$6"/Tedana/ \
	--verbose --fittype curvefit --mask "$1"/func/rest/"$6"/brain_mask.nii.gz --maxit "$4" --maxrestart "$5" --seed 42 --n-threads 4 # specify more iterations / restarts to increase likelihood of ICA convergence (also increases possible runtime).
	...
	time3=$(date "+%Y-%m-%d %H:%M:%S") && echo "${time3} ------------------sweep through files of interest"
	# sweep through files of interest
	for i in desc-ICA_components desc-limited_T2starmap desc-limited_S0map ; do
...
func $Subdir $Subject $MEPCA $MaxIterations $MaxRestarts $DataDirs
