...
SurfaceIndex = RefCifti.brainstructure > 0 & RefCifti.brainstructure < 3; % curtical
SurfaceIndex = SurfaceIndex(1:size(SurfaceCoords,1));
SurfaceCoords = SurfaceCoords(SurfaceIndex,:);
SubcorticalCoords = RefCifti.data(RefCifti.brainstructure>2,:); % subcurtical
...
parfor i = 1:length(LH_verts)
    ...
    temp_LH(:,i) = temp.cdata(LH_idx); % log distances
...
parfor i = 1:length(RH_verts)
    ...
    temp_RH(:,i) = temp.cdata(RH_idx); % log distances
...
D2 = uint8(pdist2(AllCoords,AllCoords)); % total mem up to 35GB
