function meanFD = grayplot_qa_func(Subdir)
...
%         single_echo_ori = nifti_load('/home/poolab/HeJieShi/XuZhiWei/bold_single.nii');
%         vol = reshape(single_echo_ori.vol, [], 700);
%         vol(vol<200)=0;
%         E_tSNR = nanmean(mean(vol,2) ./ std(vol,[],2));
%         E = detrend(vol','constant');
%         E = E./repmat(std(E),size(E,1),1);
%         E = E';
% 
%         single_echo_fmriprep = nifti_load('/home/poolab/HeJieShi/Rest_fmriprep.nii');
%         vol = reshape(single_echo_fmriprep.vol, [], 700);
%         F_tSNR = nanmean(mean(vol,2) ./ std(vol,[],2));
%         F = detrend(vol','constant');
...
%         D_tSNR = nanmean(mean(d.data,2) ./ std(d.data,[],2));
...
%         plot(normalize(rp-mean(rp)),'Color',[.5 .5 .5 .5]);
        ...
        title(['Head Motion (Median FD: ' num2str(median(fd),'%.2f') ', Max FD: ' num2str(max(fd),'%.2f') ', ' num2str(100 * length(find(fd < 0.2)) / length(fd),'%.2f') '% < 0.2mm)']);
...
%         xticks('');
%         set(gca,'FontName','Arial','FontSize',10,'TickLength',[0 0]);
%         title(['fmriprep (tSNR:' num2str(D_tSNR) ')']);
...
%         xticks('');
%         set(gca,'FontName','Arial','FontSize',10,'TickLength',[0 0]);
%         title(['fmriprep vol (tSNR:' num2str(F_tSNR) ')']);
...
%         xticks('');
%         set(gca,'FontName','Arial','FontSize',10,'TickLength',[0 0]);
%         title(['func (tSNR:' num2str(E_tSNR) ')']);
