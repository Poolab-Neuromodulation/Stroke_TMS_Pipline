...
for s = 1:length(sessions)
    ...
    for r = 1:length(runs)
        ...
        % load various CIFTIs;
        a = ft_read_cifti_mod([Subdir '/func/rest/session_' num2str(s) '/run_' num2str(r) '/Rest_OCME+MEICA.dtseries.nii']);
        b = ft_read_cifti_mod([Subdir '/func/rest/session_' num2str(s) '/run_' num2str(r) '/Rest_OCME+MEICA+MGTR.dtseries.nii']);
...
%         plot(normalize(rp-mean(rp)),'Color',[.5 .5 .5 .5]);
        ...
        title(['Head Motion (Median FD: ' num2str(median(fd),'%.2f') ', Max FD: ' num2str(max(fd),'%.2f') ', ' num2str(100 * length(find(fd < 1.2)) / length(fd),'%.2f') '% < 1.2mm)']);
        ...
        xticks('');
        set(gca,'FontName','Arial','FontSize',10,'TickLength',[0 0]);
        title(['single-echo after fmriprep (tSNR:' num2str(A_tSNR) ')']);
        ...
        xticks('');
        set(gca,'FontName','Arial','FontSize',10,'TickLength',[0 0]);
        title(['single-echo after MGTR (tSNR:' num2str(B_tSNR) ')']);
