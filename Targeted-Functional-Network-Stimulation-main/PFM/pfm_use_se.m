...
[C,ScanIdx,FD] = concatenate_scans(Subdir,'Rest_OCME+MEICA',1:nSessions);
...
ft_write_cifti_mod([Subdir '/func/rest/ConcatenatedCiftis/Rest_OCME+MEICA_Concatenated+SubcortRegression.dtseries.nii'],C);
...
% smoothing kernels;
for k = [0.85 1.7 2.55]
    smooth_cifti(Subdir,'Rest_OCME+MEICA_Concatenated+SubcortRegression.dtseries.nii',['Rest_OCME+MEICA_Concatenated+SubcortRegression+SpatialSmoothing' num2str(k) '.dtseries.nii'],k,k);
end

load([Subdir '/func/rest/ConcatenatedCiftis/FD']);
C = ft_read_cifti_mod([Subdir '/func/rest/ConcatenatedCiftis/Rest_OCME+MEICA_Concatenated+SubcortRegression+SpatialSmoothing2.55.dtseries.nii']);
...
pfm(C,[Subdir '/anat/T1w/fsaverage_LR32k/DistanceMatrix.mat'],[Subdir '/pfm/'],[0.0002 0.0005 0.001 0.002],[100 100 50 50],10,[],{'CORTEX_LEFT','CEREBELLUM_LEFT','ACCUMBENS_LEFT','CAUDATE_LEFT','PALLIDUM_LEFT','PUTAMEN_LEFT','THALAMUS_LEFT','HIPPOCAMPUS_LEFT','AMYGDALA_LEFT','ACCUMBENS_LEFT','CORTEX_RIGHT','CEREBELLUM_RIGHT','ACCUMBENS_RIGHT','CAUDATE_RIGHT','PALLIDUM_RIGHT','PUTAMEN_RIGHT','THALAMUS_RIGHT','HIPPOCAMPUS_RIGHT','AMYGDALA_RIGHT','ACCUMBENS_RIGHT'},8);
