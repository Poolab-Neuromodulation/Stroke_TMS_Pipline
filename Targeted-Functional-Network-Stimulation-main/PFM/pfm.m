...
ResourceDir = '/home/cputest/Stroke/Code/MultiEchofMRI-Pipeline/res0urces';
addpath(genpath(ResourceDir));

% define the infomap binary location;
InfoMap = '/home/cputest/fsl/bin/infomap';
...
% using different params, second_run cancel
    % load and trim
    % the distance matrix;
...
%     D = D(GoodVerts,GoodVerts);
    ...
    % eye() Requested 74784x74784 (41.7GB) array exceeds maximum array size preference (38.2GB).
    % m(eye(size(m,1))==1) = 0; % remove the diagonal;
    m(eye(size(m,1))==1) = 0; % remove the diagonal;
    % hejiesi: memory enough
    %     for i = 1:length(m) %did not consider condition under size(m,1) != size(m,2)
    %         m(i,i) = 0;
    %     end
    ...
    for d = 1:length(Densities)
        ...
        towrite = [x y z];
        x_len = length(x);
        clear x;clear y;clear z;clear Index;
...
% ***********************generate network data using different params, second_run cancel
% only if multiple 
...
for d = 1:length(Densities)
    ...
    for i = 1:length(uCi)
        ...
        % mark this community for removal if < 10 vertices (small communities);
...
% ft_write_cifti_mod([OutDir '/Bipartite_PhysicalCommunities' num2str(Densities(1)) '.dtseries.nii'],InfoMap_Ci);
