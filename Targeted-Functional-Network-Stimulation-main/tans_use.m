...
setenv('LD_LIBRARY_PATH', sprintf('/home/cputest/SimNIBS-4.0/simnibs_env/lib/python3.9/site-packages/simnibs/external/lib/linux:%s', getenv('LD_LIBRARY_PATH')))

% define some paths
Paths{1} = '/home/cputest/SimNIBS-4.0';
...
DataDir = '/home/poolab/HeJieShi/Health_data'; % note: should be obvious, but user needs to manually define this
...
%     FunctionalNetworks.data = FunctionalNetworks.data(:,2); % 0.005

%     FunctionalNetworks = ft_read_cifti_mod([DataDir '/' Subject '/func/rest/session_1/run_1/Rest_right_finger_motor.dtseries.nii']);
    FunctionalNetworks.time = 1;

    % isolate the target network
%     TargetNetwork = FunctionalNetworks;
%     TargetNetwork.data(TargetNetwork.data<=3.3) = 0; % note: 14 == left motor hand map.
%     TargetNetwork.data(TargetNetwork.data>3.3) = 1; % binarize.

    % isolate the target network
    TargetNetwork = FunctionalNetworks;
    TargetNetwork.data(TargetNetwork.data~=7) = 0; % note: 14 == left motor hand map.
    TargetNetwork.data(TargetNetwork.data==7) = 1; % binarize.
...
%     SearchSpace = ft_read_cifti_mod([DataDir '/' Subject '/pfm/' Subject '_Label_23.dscalar.nii']); % 23 left 57 right
    SearchSpace = ft_read_cifti_mod([DataDir '/' Subject '/pfm/' Subject '_Label_target.dscalar.nii']); % 23 left 57 right
...
%% 11
    ...
    % ROI centroid: mean(SurfaceCoords(TargetNetworkPatch.data==1,:)) = -29.4691  -15.0173   55.0499
...
%     TargetNetwork.data(TargetNetwork.data==3 | TargetNetwork.data==28) = 1; % binarize.
    ...
    tans_optimize(Subject,TargetNetworkPatch,[],PercentileThresholds,SearchGrid,DistanceToScalp,SkinSurf,VertexSurfaceArea,MidthickSurfs,WhiteSurfs,PialSurfs,MedialWallMasks,HeadMesh,AngleResolution,Uncertainty,CoilModel,OutDir,Paths);
...
%% run the "tans_dose.m" module
    ...
    MinHotSpotSize = 200; % surface area (mm2)
    
    % run the "tans_dose.m" module
    [OnTarget,Penalty,HotSpotSize] =tans_dose(magnE,VertexSurfaceArea,DiDt,AbsoluteThreshold,MinHotSpotSize,TargetNetworkPatch,[],FunctionalNetworks,NetworkLabels,[OutDir '/Optimize/'],Paths);
