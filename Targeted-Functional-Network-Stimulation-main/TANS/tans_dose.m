...
% preallocate;
OnTarget = zeros(size(magnE.data,2),1,"single"); % "OnTaget" variable (% of E-field hotspot that contains target network vertices);
Penalty = zeros(size(magnE.data,2),1,"single"); % "Penalty" variable (% of E-field hotspot that contains avoidance region / network vertices);
HotSpotSize = zeros(size(magnE.data,2),1,"single");

% preallocate;
O = TargetNetwork;
O.data = zeros(size(O.data,1),length(DiDt),"single"); % clear contents of the file;
