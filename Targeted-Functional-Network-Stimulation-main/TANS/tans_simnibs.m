...
% for i = 1:size(SearchGridCoords,1)
    ...
    try % note: sometimes the scalp position is weird and the simulation will fail.
        ...
        B = find(D < 20); % outer diameter;  % origin: 19,20; hejieshi this is the diameter if minist circle included with TargetPatch
        Circle = SkinSurf.vertices(B(~ismember(B,A)),:); % find the difference;

        % convert to a single ring (remove any "pile up");
        [~,Circle] = kmeans(Circle,round(360 / AngleResolution)); % hejieshi: convert circle(310 point) to a coarse circle(12 point-12 candidate angle)
