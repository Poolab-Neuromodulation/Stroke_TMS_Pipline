function tans_optimize(Subject,TargetNetwork,TargetNetwork2,AvoidanceRegion,PercentileThresholds,SearchGrid,DistanceToScalp,SkinFile,VertexSurfaceArea,MidthickSurfs,WhiteSurfs,PialSurfs,MedialWallMasks,HeadMesh,AngleResolution,Uncertainty,CoilModel,OutDir,Paths)
    ...
    addpath(genpath(Paths{i}));  % hejieshi: this will add python3.9 package to path, which lead writematrix ERROR!
    end
    ...
    % preallocate;
    OnTarget = zeros(length(SearchGridVertices),nCols,length(PercentileThresholds),'single'); % "OnTaget" variable (% of E-field hotspot that contains target network vertices);
    OnTarget2 = zeros(length(SearchGridVertices),nCols,length(PercentileThresholds),'single');
    Penalty = zeros(length(SearchGridVertices),nCols,length(PercentileThresholds),'single'); % "Penalty" variable (% of E-field hotspot that contains avoidance region / network vertices);
    
    % if no avoidance 
    % region is specified; 
    if isempty(AvoidanceRegion)
    AvoidanceRegion = TargetNetwork; % preallocate
    AvoidanceRegion.data = zeros(size(AvoidanceRegion.data,1),'single');
    end
...
%         system(['rm ' OutDir '/SearchGrid/' files(i).name]); % remove intermediate files;
        ...
        for ii = 1:size(MagnE.data,2)
            ...
            for iii = 1:length(PercentileThresholds)
                ...
                OnTarget2(idx,ii,iii) = (sum(VertexSurfaceArea.data(HotSpot&TargetNetwork2.data(1:59412,1)==1)) / sum(VertexSurfaceArea.data(HotSpot))); 
    ...
    save([OutDir '/Optimize/CoilCenter_OnTarget2'],'OnTarget2');
    ...
    AvgOnTarget2 = mean(OnTarget2,3); % on-target;
    ...
    AvgPenalizedOnTarget2 = mean(OnTarget2,3) - mean(Penalty,3);


    G_OnTarget = G; % preallocate;
    G_OnTarget.cdata = zeros(size(G_OnTarget.cdata),'single'); % blank slate;
    ...
    G_Penalty.cdata = zeros(size(G_OnTarget.cdata),'single'); % blank slate;
    ...
    G_PenalizedOnTarget.cdata = zeros(size(G_OnTarget.cdata),'single'); % blank slate;
    ...
    PenalizedOnTarget_ErrorAdjusted = zeros(length(SearchGridVertices),1,'single');
    ...
    Circle = Skin.vertices(B(~ismember(B,A)),:); % hejieshi: same as 'tabs_simnibs.m' line54, find the difference; 19<Circle<20
    ...
    % preallocate;
    OnTarget = zeros(1,nCols,length(PercentileThresholds),'single'); % "OnTaget" variable (% of E-field hotspot that contains target network vertices);
    OnTarget2 = zeros(1,nCols,length(PercentileThresholds),'single');
    Penalty = zeros(1,nCols,length(PercentileThresholds),'single'); % "Penalty" variable (% of E-field hotspot that contains avoidance region / network vertices);
    ...
    for i = 1:size(MagnE.data,2)
        ...
        for ii = 1:length(PercentileThresholds)
            ...
            OnTarget2(1,i,ii) = (sum(VertexSurfaceArea.data(HotSpot&TargetNetwork2.data(1:59412,1)==1)) / sum(VertexSurfaceArea.data(HotSpot)));
    ...
    save([OutDir '/Optimize/CoilOrientation_OnTarget2'],'OnTarget2');
    save([OutDir '/Optimize/CoilOrientation_Penalty'],'Penalty');
    load([OutDir '/Optimize/CoilOrientation_OnTarget']);
    load([OutDir '/Optimize/CoilOrientation_OnTarget2']);
    load([OutDir '/Optimize/CoilOrientation_Penalty']);
    
    % average accross the e-field thresholds;
    AvgOnTarget = mean(OnTarget,3); % on-target;
    AvgOnTarget2 = mean(OnTarget2,3);
    AvgPenalty = mean(Penalty,3); % penalty;
    ...
    G.cdata = zeros(size(G.cdata),'single'); % blank slate;
    ...
    G.cdata(RefDirsVertices) = AvgOnTarget2; % 
    save(G,[OutDir '/Optimize/CoilOrientation_OnTarget2.shape.gii']); % write out the on-target metric file;
    system(['wb_command -metric-dilate ' OutDir '/Optimize/CoilOrientation_OnTarget2.shape.gii ' SkinFile ' 2  ' OutDir '/Optimize/CoilOrientation_OnTarget2.shape.gii -nearest']);
    system(['wb_command -metric-smoothing ' SkinFile ' ' OutDir '/Optimize/CoilOrientation_OnTarget2.shape.gii ' num2str(SmoothingFactor) ' ' OutDir '/Optimize/CoilOrientation_OnTarget2_s' num2str(SmoothingFactor) '.shape.gii -fix-zeros']);
    ...
    G.cdata(RefDirsVertices) = AvgPenalizedOnTarget2; % 
    save(G,[OutDir '/Optimize/CoilOrientation_PenalizedOnTarget2.shape.gii']); % write out the on-target metric file;
    system(['wb_command -metric-dilate ' OutDir '/Optimize/CoilOrientation_PenalizedOnTarget2.shape.gii ' SkinFile ' 2  ' OutDir '/Optimize/CoilOrientation_PenalizedOnTarget2.shape.gii -nearest']);
    system(['wb_command -metric-smoothing ' SkinFile ' ' OutDir '/Optimize/CoilOrientation_PenalizedOnTarget2.shape.gii ' num2str(SmoothingFactor) ' ' OutDir '/Optimize/CoilOrientation_PenalizedOnTarget2_s' num2str(SmoothingFactor) '.shape.gii -fix-zeros']);
    
    finalOnTarget = 1*AvgPenalizedOnTarget + 0.5*AvgPenalizedOnTarget2;
    [~,idx] = sort(AvgPenalizedOnTarget, 'descend');
    Idx = find(AvgPenalizedOnTarget2==max(AvgPenalizedOnTarget2(idx(1:5))));

    % find the coil orientation that has
    % the relative on-target score (on average);
%     Idx = find(AvgPenalizedOnTarget2==max(AvgPenalizedOnTarget2));
%     Idx = find(finalOnTarget==max(finalOnTarget));
    ...
    system(['echo ' num2str(Idx(1)) ' >> ' OutDir '/Optimize/best_orient.txt']);
    ...
    % % ************************************************hejieshi second run apply(because siminbs/python3.9)*******************************************************
    % load([OutDir '/Optimize/CoilOrientation_OnTarget']);
    % load([OutDir '/Optimize/CoilOrientation_Penalty']);
    % AvgOnTarget = mean(OnTarget,3); % on-target;
    % AvgPenalty = mean(Penalty,3); % penalty;
    ...
    writematrix(M{Idx(1)},[OutDir '/Optimize/' CoilName '_xfm.txt'],'Delimiter','\t');
    ...
    % preallocate;
    SortedCoords = zeros(size(Coords),'single');
    ...
    % preallocate;
    Output = zeros(size(Coords),'single');
