...
% hejieshi: TargetNetwork.brainstructure(VertexSurfaceArea.pos(:,3)>30) = -1;  % remove surface whose z coordinary is much more than 30
% O.data(VertexSurfaceArea.pos(:,3)>20) = 0;  % remove surface whose z coordinary is much more than 30
% LH.vertices(LH.vertices(:,3)>30,:) = [];
% RH.vertices(RH.vertices(:,3)>30,:) = [];
...
if isempty(SearchSpace)
    ...
    SearchSpace.data = ones(size(SearchSpace.data,1),1);   
end
...
%         ClusterSize(i) =  sum(VertexSurfaceArea.data(find(Clusters.data==uClusters(i))));
...
% [~, sort_idx] = sort(ClusterSize, 'descend');

% write out the target network patch 
O.data = zeros(size(O.data,1),1); % blank slate
O.data(TargetPatch==1)=1; % binarize;
% add by hejieshi: calculate biggest two cluster
% ClusterSize =   916.3305,2.4963,997.8892, so TargetPatch = 1,3
% TargetPatch = sort_idx(2);
% TargetPatch = Clusters.data(1:59412)==TargetPatch;
% O.data(TargetPatch==1)=1; % binarize;
